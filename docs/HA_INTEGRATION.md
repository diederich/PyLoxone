...
**Meter discovery walks subControls and links via `via_device`.** `Meter` controls are surfaced as their own device with up to four subsensors. Discovery uses `helpers.get_all_including_subcontrols("Meter", with_parent=True)` so meters nested inside a parent control — most commonly the per-circuit meters of a `PowerUnit` (Loxone Energy Flow Monitor) — are exposed alongside top-level meters. Subcontrol meters inherit `room` and `cat` from their parent when not set on the subcontrol itself, and each sub-meter `DeviceInfo` carries `via_device=(DOMAIN, parent_uuid)` so HA renders them visually nested under the parent PowerUnit. The setup pre-registers each `PowerUnit` parent device via `dr.async_get_or_create` before adding entities, because HA silently drops `via_device` links whose target isn't already in the device registry.

**Meter sub-sensor labels depend on the meter type.** Loxone's `total` / `totalNeg` semantic flips for storage meters: for a normal energy (consumer / grid) meter `total` is the import accumulator and `totalNeg` is the export accumulator; for a storage / battery meter (`details.type == "storage"`, e.g. BYD HVB) `total` accumulates **discharge** (Loxone `Mrd`) and `totalNeg` accumulates **charge** (Loxone `Mrc`). The single-table mapping used to label both as `Total` / `Total Returned`, which silently swaps the Home Assistant Energy Dashboard "Battery storage" source for new users.

`sensor._meter_state_layout(meter_type)` returns the right `(state_key, name_suffix, translation_key, format_key)` tuples per meter type. Storage meters use `meter_storage_discharge` ("Discharge energy"), `meter_storage_charge` ("Charge energy"), `meter_storage_level` ("Battery level"); `actual` stays on the shared `meter_actual` key (Loxone's signed power convention `+W = discharge` already matches the HA Energy / power-flow card convention, so no rename is needed). `_METER_CLASSIFICATION["storage"]` ensures the storage SoC sub-sensor lands on `device_class=battery` even if its format string doesn't trigger keyword matching, and `_METER_EXPECTED_CLASSES["storage"]` extends the unit-mismatch repair check to storage meters.

For users who installed the integration **before** the storage relabel, `_maybe_create_storage_legacy_repair` raises a non-fixable `WARNING` Repair issue once per storage meter when their `total` / `totalNeg` entities still carry the legacy `..._total` / `..._total_returned` slugs. The issue lists the actual entity IDs and the correct Energy Dashboard mapping (discharge -> "Energy from battery", charge -> "Energy to battery") and is dismissable. Fresh installs land directly on the new slugs (`..._discharge_energy` / `..._charge_energy`) and never see the warning. Entity IDs are intentionally NOT renamed automatically, because that would break any existing automations / Lovelace cards / template sensors pointing at the old IDs — users can rename via Settings if they want.
