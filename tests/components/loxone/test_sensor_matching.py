"""Pure-unit tests for Loxone sensor classification and unit extraction.

These tests exercise `match_sensor_description`, `UNAMBIGUOUS_UNITS`, and
`clean_unit` directly — no HA fixtures, no integration setup. They guard the
classification table independent of the rest of the integration.
"""

from custom_components.loxone.helpers import clean_unit
from custom_components.loxone.sensor import SENSOR_TYPES, UNAMBIGUOUS_UNITS, match_sensor_description
...
class TestSensorMatching:
    """Test the single classification entry point."""
    ...
    def test_temperature_same_description(self):
        ...
        assert match_sensor_description(unit=UnitOfTemperature.CELSIUS) is match_sensor_description(
    ...
    def test_power_same_description(self):
        ...
        assert match_sensor_description(unit=UnitOfPower.WATT) is match_sensor_description(
    ...
    def test_cubic_meters_no_match(self):
        ...
        assert match_sensor_description(unit=UnitOfVolume.CUBIC_METERS) is None
    ...
    def test_no_match_unknown_unit(self):
        assert match_sensor_description(unit="foo") is None

    def test_no_match_percentage_unknown_context(self):
        """% without matching name/category keywords returns None."""
        assert match_sensor_description(unit=PERCENTAGE, name="Random", category="Unknown") is None
...
class TestUnambiguousUnits:
    """UNAMBIGUOUS_UNITS should be derived purely from keyword-free descriptions."""
...
class TestUnitExtraction:
    """clean_unit() pulls the trailing unit from a Loxone format string."""
    ...
    def test_extract_bare_format(self):
        """Format string without a unit returns an empty string."""
...
# ============================================================================
# SENSOR_TYPES table structure
# ============================================================================


class TestSensorTypesStructure:
    """SENSOR_TYPES should be classification-only and well-formed."""

    def test_every_description_has_device_class(self):
        """No description should be missing a device_class (regression: old humidity_or_battery)."""
    ...
    def test_no_native_unit_in_descriptions(self):
        """Descriptions carry classification only; unit comes from _attr_* in __init__."""
        for desc in SENSOR_TYPES:
