...
async def test_diagnostics_redacts_credentials(hass: HomeAssistant, init_integration: MockConfigEntry) -> None:
...
async def test_diagnostics_includes_connection_state(hass: HomeAssistant, init_integration: MockConfigEntry) -> None:
    """Diagnostics should include the current connection state."""
    diag = await async_get_config_entry_diagnostics(hass, init_integration)
    assert diag["connection_state"] == "connected"


async def test_diagnostics_includes_miniserver_info(hass: HomeAssistant, init_integration: MockConfigEntry) -> None:
...
async def test_diagnostics_redacts_structure(hass: HomeAssistant, init_integration: MockConfigEntry) -> None:
