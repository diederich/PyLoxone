...
@pytest.fixture
def coordinator(hass: HomeAssistant, mock_config_entry: MockConfigEntry) -> LoxoneCoordinator:
...
async def test_structure_poll_detects_change(hass: HomeAssistant, coordinator: LoxoneCoordinator) -> None:
...
async def test_structure_poll_no_change(hass: HomeAssistant, coordinator: LoxoneCoordinator) -> None:
...
async def test_structure_poll_http_error_ignored(hass: HomeAssistant, coordinator: LoxoneCoordinator) -> None:
...
async def test_structure_poll_skips_when_disconnected(hass: HomeAssistant, coordinator: LoxoneCoordinator) -> None:
    """Poll callback should skip when not connected."""
    coordinator.connection_state = ConnectionState.RECONNECTING

    with patch("custom_components.loxone.coordinator.async_get_clientsession") as mock_fn:
